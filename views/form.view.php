...
<div class="row">
  ...
  <div class="col-md-6">
    ...
    <form action="#" method="post" id="form-main">  
      ...
      <textarea name="excel_input" id="excel-input" class="form-control form-save"><?= @$_POST['excel_input']; // Prints input after form submitting ?></textarea>
  ...
  <div class="col-md-6">
    ...
    <textarea name="sql_output" id="sql-output" class="form-control form-save" readonly><?= is_a($this->data, 'models\Table') ? $this->data->toSql() : '' // Generated SQL queries ?></textarea>

    
    <?php if(!empty($this->data->rows)){ // If there are any processed data rows... ?>
      <div class="right">
        <small>
          Generate <?= count($this->data->rows); // Count of data rows ?> queries took <?= round(elapsedTime(), 3); // Elapsed time from script start ?> seconds.
        </small>
      </div>
    <?php } ?>
